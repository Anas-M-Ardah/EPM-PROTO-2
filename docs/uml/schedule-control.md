...
| 4 | **No page-head Export / Import P6 buttons.** The reference carries both. | Demo toasts, and the built Contracts and Alerts pages omit the same pattern. Restored as demo toasts at Phase 2.9 (P-34). **Export is not Phase 2.6 after all** — v1.1's `DReports` is a report catalog with no Export button (P-37); the P6 import parser is explicitly out of scope (`07 §8`). |
