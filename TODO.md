...
| P-194 | ملحق الشكل 23 prints **«أصبحت حرجة»** — how many activities the slip PUT on the critical path. Separating those from the ones that were ALWAYS critical needs the baseline's own float, and **`EP-SCD-06` deliberately does not record float at «تثبيت خط الأساس»** (المسار 4 step 8): its comment says criticality belongs to the schedule in force and a re-baseline is not a reset. Should fixing a baseline freeze float too? | Counted as «critical AND slipped» through `ScheduleImpact.FloatBefore`, which is ملحق الشكل 23's own arithmetic — before = after + slip, per its A2 worked example «الأساس … عوم 7 · الحالي … عوم 2» over a 5-day slip. The limit is stated in `ScheduleEndpoints` rather than hidden. **The reference cannot compute it either**: its `comparison.nowCritical` is fixture data |
